...
close all
format short
%% Thermal neutron (from sphere source) transport in light water 
...
for j = 1:25 %no of samples
    ...
    for i = 1:nparticles
        %neutron source loc: sphere of rad = 1
        r0 = 1*((rand())^(1/3));
        theta0 = acos(1-2*rand());
        phi0 = 2*pi*rand();
        x(i) = r0*sin(theta0)*cos(phi0);
        y(i) = r0*sin(theta0)*sin(phi0);
        z(i) = r0*cos(theta0);
        xs(i) = x(i);
        ys(i) = y(i);
        zs(i) = z(i);
        ...
        r(i) = sqrt((x(i)-xs(i))^2 + (y(i)-ys(i))^2 + (z(i)-zs(i))^2);
